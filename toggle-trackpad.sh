...
[ -x /usr/bin/xinput ] || {
  echo "No \`xinput' binary on $(hostname) - nothing to do!" >&2
  exit 1
}
...
[ -n "$ID" ] || {
  echo "Can't work out Touchpad ID via \'xinput list'!" >&2
  exit 2
}
...
[ -n "$CURRENT_STATE" ] || {
  echo "Can't work out current state of ID=$ID via \'xinput list-props $ID'!" >&2
  exit 2
}
